...
/* Category Scroll Pagination */
  _scrollListenerCategory() async {
    ...
    if (categoryController.offset >= categoryController.position.maxScrollExtent &&
        !categoryController.position.outOfRange &&
        (homeProvider.categorycurrentPage ?? 0) < (homeProvider.categorytotalPage ?? 0)) {
...
/* Playlist Pagination */
  _scrollListenerPlaylist() async {
    ...
    if (playlistController.offset >= playlistController.position.maxScrollExtent &&
        !playlistController.position.outOfRange &&
        (homeProvider.playlistcurrentPage ?? 0) < (homeProvider.playlisttotalPage ?? 0)) {
...
/* Category Api  */
  Future<void> _fetchDataCategory(int? nextPage) async {
    await homeProvider.getHashTagTitles();
  }
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 2, // Two tabs: Hashtags and Trending
      child: Scaffold(
        backgroundColor: Colors.black,
        appBar: AppBar(
          backgroundColor: Colors.black,
          foregroundColor: Colors.white,
          title: const CustomAppBar(contentType: "1"),
          bottom: const TabBar(
            labelColor: Colors.white, // Active tab text color (white)
            unselectedLabelColor: Colors.grey, // Inactive tab text color (gray)
  ...
  Widget BuildHashtag() {
    return Consumer<HomeProvider>(builder: (context, categoryprovider, child) {
      ...
      itemBuilder: (context, index) {
        ...
        return Align(
          alignment: Alignment.center, // Align the Container to the left
          child: Container(
            margin: EdgeInsets.all(5),
            width: screenWidth * 0.5, // Set width to 30% of screen width
            padding: EdgeInsets.all(1), // Optional padding
            ...
            child: Center(
              child: Text(
                "# " + homeProvider.HashTag?[index].name,
                style: TextStyle(color: Colors.white),
              ),
            ),
          ),
  ...
  Widget buildCategory() {
    return Consumer<HomeProvider>(builder: (context, categoryprovider, child) {
      if (categoryprovider.categoryloading && !categoryprovider.categoryloadMore) {
  ...
  Widget videocategoryList() {
    if (homeProvider.categorymodel.status == 200 && homeProvider.categorydataList != null) {
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return InkWell(
                ...
                onTap: () async {
                  homeProvider.selectCategory(index, homeProvider.categorydataList?[index].id.toString());
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                      color: homeProvider.catindex == index ? colorAccent : colorPrimary,
 ...
 Widget categoryShimmer2() {
    ...
    return Consumer<HomeProvider>(builder: (context, videolistprovider, child) {
      if (videolistprovider.videoloading && !videolistprovider.videoloadMore) {
        ...
        return Column(
          children: [
            allVideo(),
            if (homeProvider.videoloadMore) SizedBox(height: 50, child: Utils.pageLoader(context)) else const SizedBox.shrink(),
          ],
        );
      }
    });
  ...
  Widget allVideo() {
    if (homeProvider.videolistmodel.status == 200 && homeProvider.videoList != null) {
      if ((homeProvider.videoList?.length ?? 0) > 0) {
                  ...
                  MaterialPageRoute(
                    builder: (context) {
                      return Detail(
                        ...
                        videoid: homeProvider.videoList?[index].id.toString() ?? "",
                      );
                    },
                  ),
                );
                  ...
                  Stack(
                    children: [
                      MyNetworkImage(
                          ...
                          fit: BoxFit.fill,
                          height: 240,
                          imagePath: homeProvider.videoList?[index].landscapeImg.toString() ?? ""),
                      Positioned.fill(
                        right: 15,
                        bottom: 15,
                        child: Align(
                          ...
                          child: Container(
                            ...
                            child: MyText(
                                color: white,
                                text: Utils.formatTime(double.parse(homeProvider.videoList?[index].contentDuration.toString() ?? "")),
                                textalign: TextAlign.center,
                                fontsize: Dimens.textSmall,
                                inter: false,
                                multilanguage: false,
                      ...
                      children: [
                        ClipRRect(
                          ...
                          child: MyNetworkImage(
                            width: 35,
                            height: 35,
                            imagePath: homeProvider.videoList?[index].portraitImg.toString() ?? "",
                            fit: BoxFit.cover,
                          ),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              MyText(
                                  color: white,
                                  text: homeProvider.videoList?[index].title.toString() ?? "",
                              ...
                              RichText(
                                text: TextSpan(
                                  text: "${homeProvider.videoList?[index].channelName.toString() ?? ""}  ",
                                  style: Utils.googleFontStyle(1, Dimens.textSmall, FontStyle.normal, gray, FontWeight.w500),
                                  children: [
                                    TextSpan(
                                      text: "${Utils.kmbGenerator(homeProvider.videoList?[0].totalView ?? 0)} ",
                                      style: Utils.googleFontStyle(1, Dimens.textSmall, FontStyle.normal, gray, FontWeight.w500),
                                    ),
                                    TextSpan(
                                      text: 'views ',
                                    ...
                                    TextSpan(
                                      text: Utils.timeAgoCustom(
                                        DateTime.parse(
                                          homeProvider.videoList?[index].createdAt ?? "",
                                        ),
                                      ),
                                      style: Utils.googleFontStyle(1, Dimens.textSmall, FontStyle.normal, gray, FontWeight.w500),
                                    ),
                        ...
                        InkWell(
                          onTap: () {
                            if (Constant.userID == null) {
                              ...
                              moreBottomSheet(
                                homeProvider.videoList?[index].userId.toString() ?? "",
        ...
        return const NoData(title: "nodatavideotitle", subTitle: "nodatavideosubtitle");
      }
    } else {
      return const NoData(title: "nodatavideotitle", subTitle: "nodatavideosubtitle");
    }
          ...
          child: Wrap(
            children: [
              Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  moreFunctionItem("ic_watchlater.png", "savetowatchlater", () async {
                    await homeProvider.addremoveWatchLater("1", contentid, "0", "1");
                    if (!mounted) return;
                    Navigator.of(context).pop();
                    Utils.showSnackbar(context, "savetowatchlater");
                  }),
                  moreFunctionItem("ic_playlisttitle.png", "savetoplaylist", () async {
...
/* Report Reason Bottom Sheet */
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: () async {
                        if (homeProvider.reasonId == "" || homeProvider.reasonId.isEmpty) {
                          Utils.showSnackbar(context, "pleaseselectyourreportreason");
                        } else {
                          await homeProvider.addContentReport(
                              reportUserid, contentid, homeProvider.reportReasonList?[homeProvider.reportPosition ?? 0].reason.toString() ?? "", "1");
                          if (!mounted) return;
                          Navigator.pop(context);
                      ...
                      child: Container(
                        padding: const EdgeInsets.fromLTRB(15, 10, 15, 10),
                        decoration: BoxDecoration(color: colorAccent, borderRadius: BorderRadius.circular(5)),
                        child: MyText(
                            color: white,
                            text: "report",
                            textalign: TextAlign.left,
  ...
  Widget buildReportReasonList() {
    return Consumer<HomeProvider>(builder: (context, reportreasonprovider, child) {
      if (reportreasonprovider.getcontentreportloading && !reportreasonprovider.getcontentreportloadmore) {
  ...
  Widget buildReportReasonListItem() {
    ...
    if (homeProvider.getRepostReasonModel.status == 200 && homeProvider.reportReasonList != null) {
      if ((homeProvider.reportReasonList?.length ?? 0) > 0) {
        return ListView.builder(
          ...
          itemBuilder: (BuildContext ctx, index) {
            return InkWell(
              onTap: () {
                homeProvider.selectReportReason(index, true, homeProvider.reportReasonList?[index].id.toString() ?? "");
              },
              child: Container(
                padding: const EdgeInsets.fromLTRB(10, 0, 10, 0),
                color: homeProvider.reportPosition == index && homeProvider.isSelectReason == true ? colorAccent : colorPrimaryDark,
                height: 45,
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.start,
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    ...
                    Expanded(
                      child: MyText(
                          color: white,
                          text: homeProvider.reportReasonList?[index].reason.toString() ?? "",
                          textalign: TextAlign.left,
                          fontsize: Dimens.textTitle,
                          multilanguage: false,
                          inter: false,
                    ...
                    const SizedBox(width: 20),
                    homeProvider.reportPosition == index && homeProvider.isSelectReason == true
          ...
          return Container(
            ...
            child: Column(
              children: [
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      InkWell(
                        onTap: () async {
                          Navigator.pop(context);
                          createPlaylistDilog(playlistId: homeProvider.playlistId);
                        },
                        child: Container(
                          width: 160,
                          padding: const EdgeInsets.fromLTRB(0, 5, 0, 5),
                          decoration: BoxDecoration(border: Border.all(width: 1, color: colorAccent), borderRadius: BorderRadius.circular(5)),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            crossAxisAlignment: CrossAxisAlignment.center,
                            children: [
                ...
                Consumer<HomeProvider>(builder: (context, playlistprovider, child) {
                  if (playlistprovider.playlistLoading && !playlistprovider.playlistLoadmore) {
                    return const SizedBox.shrink();
                  } else {
                    if (homeProvider.getContentbyChannelModel.status == 200 && homeProvider.playlistData != null) {
                        ...
                        return Row(
                          ...
                          children: [
                            ...
                            InkWell(
                              onTap: () async {
                                Navigator.pop(context);
                                if (homeProvider.playlistId.isEmpty || homeProvider.playlistId == "") {
                                  Utils.showSnackbar(context, "pleaseelectyourplaylist");
                                } else {
                                  await homeProvider.addremoveContentToPlaylist(
                                      Constant.channelID,
                                      homeProvider.getContentbyChannelModel.result?[homeProvider.playlistPosition ?? 0].id.toString() ?? "",
                                      "1",
                                      contentid,
                                      "0",
                                      "1");

                                  if (!homeProvider.addremovecontentplaylistloading) {
                                    if (homeProvider.addremoveContentToPlaylistModel.status == 200) {
                                      debugPrint("Added Succsessfully");
                                      Utils().showToast("Save to Playlist");
                                    } else {
                                      Utils().showToast("${homeProvider.addremoveContentToPlaylistModel.message}");
                                    }
                                  }
                                }
                              },
      ...
      if ((homeProvider.playlistData?.length ?? 0) > 0) {
        return ListView.builder(
          ...
          itemBuilder: (BuildContext ctx, index) {
            return InkWell(
              onTap: () {
                homeProvider.selectPlaylist(index, homeProvider.playlistData?[index].id.toString() ?? "", true);
              },
              child: Container(
                padding: const EdgeInsets.fromLTRB(15, 0, 15, 0),
                color: homeProvider.playlistPosition == index && homeProvider.isSelectPlaylist == true ? colorAccent : colorPrimaryDark,
                height: 45,
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.start,
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    ...
                    Expanded(
                      child: MyText(
                          color: white,
                          text: homeProvider.playlistData?[index].title.toString() ?? "",
                          textalign: TextAlign.left,
                          fontsize: Dimens.textTitle,
                          multilanguage: false,
                          inter: false,
                    ...
                    const SizedBox(width: 20),
                    homeProvider.playlistPosition == index && homeProvider.isSelectPlaylist == true
                        ? MyImage(width: 18, height: 18, imagePath: "true.png")
                        : const SizedBox.shrink(),
                  ],
                ),
        ...
        return const NoData(title: "noplaylistfound", subTitle: "createnewplaylist");
      }
    } else {
      return const NoData(title: "noplaylistfound", subTitle: "createnewplaylist");
    }
      ...
      builder: (BuildContext context) {
        return Dialog(
          ...
          child: Container(
            ...
            child: Consumer<HomeProvider>(builder: (context, createplaylistprovider, child) {
              return Column(
                mainAxisAlignment: MainAxisAlignment.center,
                crossAxisAlignment: CrossAxisAlignment.center,
                children: [
                    ...
                    children: [
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          height: 20,
                          decoration: BoxDecoration(
                            color: createplaylistprovider.isType == 1 ? colorAccent : transparent,
                            border: Border.all(width: 2, color: createplaylistprovider.isType == 1 ? colorAccent : white),
                          ),
                      ...
                      InkWell(
                        ...
                        child: Container(
                          ...
                          height: 20,
                          decoration: BoxDecoration(
                            color: createplaylistprovider.isType == 2 ? colorAccent : transparent,
                            border: Border.all(width: 2, color: createplaylistprovider.isType == 2 ? colorAccent : white),
                          ),
                      ...
                      InkWell(
                        onTap: () async {
                          if (playlistTitleController.text.isEmpty) {
                            Utils.showSnackbar(context, "pleaseenterplaylistname");
                          } else if (createplaylistprovider.isType == 0) {
                            Utils.showSnackbar(context, "pleaseselectplaylisttype");
                          } else {
                            ...
                            if (!createplaylistprovider.loading) {
                              if (createplaylistprovider.createPlaylistModel.status == 200) {
                                if (!mounted) return;
                                Utils.showSnackbar(context, "playlistcreatesuccsessfully");
                              } else {
                                if (!mounted) return;
                                Utils.showSnackbar(context, "${createplaylistprovider.createPlaylistModel.message}");
                              }
                            }
                            if (!mounted) return;
                            Navigator.pop(context);
